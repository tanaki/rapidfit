...
type Tab = 'video' | 'company' | 'interface';
...
export function SettingsModal({ config, onChange, onClose, devices, company, onCompany }: Props) {
  ...
  return (
    ...
    >
      <div className="bg-[#13131f] border border-[#22223b] rounded-xl p-6 w-[560px] max-h-[90vh] flex flex-col shadow-2xl">
        ...
        {/* ── Onglets ── */}
        <div className="flex gap-1 bg-[#0e0e1a] rounded-lg p-1 mb-5">
          {tabs.map(({ id, label }) => (
            <button key={id} onClick={() => setTab(id)}
              className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-colors ${
                tab === id ? 'bg-[#22223b] text-slate-100' : 'text-slate-500 hover:text-slate-300'
              }`}>
              {label}
            </button>
          ))}
        ...
        <button onClick={handleApply}
          className="mt-6 w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-medium transition-colors shrink-0">
          {t('settings.apply')}
        </button>
      </div>
    </div>
