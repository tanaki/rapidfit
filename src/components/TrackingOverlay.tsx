...
export function TrackingOverlay({ mode, initIndex, clickedPoints, videoRect, onInitClick, onStop }: Props) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Ignore les clics hors de la zone vidéo
  ...
  return (
    <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 50 }}>
      ...
      {/* Intercepteur de clics — actif uniquement en mode initializing */}
      {mode === 'initializing' && (
        <div
      ...
      {/* Dots pour les joints déjà placés */}
      ...
      {mode === 'initializing' && nextJointLabel && (
        <div
          ...
          style={{ left: panelLeft, top: panelTop, transform: 'translateX(-50%)' }}
        >
          <div className="bg-black/75 text-white rounded-xl px-4 py-2.5 text-center backdrop-blur-sm shadow-xl">
            ...
            <div className="text-sm font-semibold text-yellow-300">{nextJointLabel}</div>
          </div>
        </div>
      )}
